...
void Player::DeleteFromList(int Index, int &cnt)
{
    ...
    int count_begin = MediaPlayerlist.mediaCount();
    for(int i = 0; i < count_begin ; i++)
        qDebug() << MediaPlayerlist.media(i).canonicalUrl();

    //MediaPlayerlist.setCurrentIndex(-1);
    qDebug() << "delete index " << Index;
    qDebug() << "current index" << MediaPlayerlist.currentIndex();
    qDebug() << "current size" << MediaPlayerlist.mediaCount();
